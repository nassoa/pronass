...
export default function Cursor() {
  ...
  useEffect(() => {
    ...
    /* ── Compression au clic ── */
    const onDown = () => arrow.classList.add("click");
    const onUp = () => arrow.classList.remove("click");

    document.addEventListener("mousemove", onMove);
    document.addEventListener("mousedown", onDown);
    document.addEventListener("mouseup", onUp);

    /* ── Main au survol : bind une seule fois par élément ──
       WeakSet pour tracker sans créer de memory leak        */
    const bound = new WeakSet<Element>();

    const bindEl = (el: Element) => {
      if (bound.has(el)) return;
      bound.add(el);
      el.addEventListener("mouseenter", () => arrow.classList.add("pointer"));
    ...
    /* Observer les éléments ajoutés dynamiquement par Next.js */
  ...
  return (
    <div ref={arrowRef} id="cur-arrow">
      {/* Flèche — visible par défaut */}
      <svg
      ...
      >
        <path
          d="M3.5 1.5L3.5 17.5L8 13L11 19.5L13 18.5L10 12L16.5 12L3.5 1.5Z"
